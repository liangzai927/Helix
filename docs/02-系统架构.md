...
flowchart LR
    ...
    VscodeUI --> Protocol["packages/protocol<br/>@helix-agent/protocol<br/>AgentEvent / Message / AgentPlan / AgentPatch"]
    CliUI --> Protocol
    DesktopUI --> Protocol
...
这是协议层，当前包名是 `@helix-agent/protocol`。
...
按照当前 `packages/protocol/src/types/event.ts` 的定义，一个用户任务在事件流里的典型过程可以是：
